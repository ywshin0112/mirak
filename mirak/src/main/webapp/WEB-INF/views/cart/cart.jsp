...
<section class="ftco-section">
                        ...
                        <c:forEach var="c" items="${cartList }">
                           <tr class="text-center">
                              <td class="cart_info"><input type="checkbox"
                                 ...
                                 value="${c.cart_code }"> <input type="hidden"
                                 class="price_input" value="${c.pro_price }"> <input
                                 type="hidden" class="count_input" value="${c.cart_cnt }">
                                 <input type="hidden" class="totalPrice_input"
                                 value="${c.pro_price * c.cart_cnt }"> <input
                                 type="hidden" class="pro_code" value="${c.pro_code }">
                                 <input type="hidden" class="cart_code"
                                 value="${c.cart_code }"></td>
                              <td class="image-prod">
                                 <div>
                                    <img alt="1"
                              ...
                              <td><input type="button" value="변경"
                                 ...
                                 data-target="#modal${c.cart_code }"></td>
                              <td class="product-remove"><a
                                 href="/cart/cartDelete/${c.cart_code }"
   ...
   <!-- Modal -->
   <c:forEach var="c" items="${cartList}">
      <div class="modal fade" id="modal${c.cart_code }" tabindex="9999"
         aria-labelledby="exampleModalLabel" aria-hidden="true">
         <div class="modal-dialog modal-lg">
            <div class="modal-content">
               <form action="/cartUpdate" method="post" name="day"
                  onsubmit="return daycheck(this)">
                  <input type="hidden" name="cart_code" value="${c.cart_code }">
                  <input type="hidden" name="cart_totprice"
                     value="${c.cart_cnt * c.pro_price}">
                        ...
                        <div class="blog-entry align-self-stretch d-md-flex">
                           <div>
                              <img src="${path}/resources/images/product/${c.pro_image}"
                                 title="${c.pro_name }" alt="${c.pro_desc }"
                                 style="width: 250px;">
                           </div>
                           <div class="text d-block pl-md-4">
                              <h3 class="heading">${c.pro_name }</h3>
                              <p>
                                 <span class="font-weight-bold text-dark">상품 상세</span>
                                 ${c.pro_desc}
                              </p>
                              <p>
                              ...
                              <p>
                                 <span class="font-weight-bold text-dark">상품 가격</span>
                                 ${c.pro_price }원 <br>
                              </p>
                              <p>
                                 <span class="font-weight-bold text-dark">배송 시작일</span> <input
                                    type="date" id="start" name="cart_start"
                                    class="form-control input-number" value="${c.cart_start }"
                                    required="required">
                              </p>
                              <p>
                                 <span class="font-weight-bold text-dark">배송 요일</span><br>
                                 &nbsp; <label for="mon"><input type="checkbox"
                                    name="cart_day" id="mon" style="transform: scale(1.5);"
                                    value="월"
                                    <c:if test = "${fn : contains(c.cart_day, '월')}">checked</c:if>>&nbsp;&nbsp;월</label>&nbsp;&nbsp;
                                 <label for="tue"><input type="checkbox"
                                    name="cart_day" id="tue" style="transform: scale(1.5);"
                                    value="화"
                                    <c:if test = "${fn : contains(c.cart_day, '화')}">checked</c:if>>&nbsp;&nbsp;화</label>&nbsp;&nbsp;
                                 <label for="wed"><input type="checkbox"
                                    name="cart_day" id="wed" style="transform: scale(1.5);"
                                    value="수"
                                    <c:if test = "${fn : contains(c.cart_day, '수')}">checked</c:if>>&nbsp;&nbsp;수</label>&nbsp;&nbsp;
                                 <label for="thu"><input type="checkbox"
                                    name="cart_day" id="thu" style="transform: scale(1.5);"
                                    value="목"
                                    <c:if test = "${fn : contains(c.cart_day, '목')}">checked</c:if>>&nbsp;&nbsp;목</label>&nbsp;&nbsp;
                                 <label for="fri"><input type="checkbox"
                                    name="cart_day" id="fri" style="transform: scale(1.5);"
                                    value="금"
                                    <c:if test = "${fn : contains(c.cart_day, '금')}">checked</c:if>>&nbsp;&nbsp;금</label>&nbsp;&nbsp;
                                 <label for="sat"><input type="checkbox"
                                    name="cart_day" id="sat" style="transform: scale(1.5);"
                                    value="토"
                                    <c:if test = "${fn : contains(c.cart_day, '토')}">checked</c:if>>&nbsp;&nbsp;토</label>&nbsp;&nbsp;
                                 <label for="sun"><input type="checkbox"
                                    name="cart_day" id="sun" style="transform: scale(1.5);"
                                    value="일"
                                    <c:if test = "${fn : contains(c.cart_day, '일')}">checked</c:if>>&nbsp;&nbsp;일</label>

                                 <label for="all"><input type="checkbox"
                                    name="cart_all" id="all" style="transform: scale(1.5);"
                                    value="all" onclick='selectAll(this)'>&nbsp;&nbsp;전체선택</label>
                                 &nbsp;&nbsp; <label for="pyeong"><input
                                    type="checkbox" name="cart_pyeong" id="pyeong"
                                    style="transform: scale(1.5);" value="pyeong"
                                    onclick='selectpyeong(this)'>&nbsp;&nbsp;주중선택</label>
                                 &nbsp;&nbsp; <label for="jumal"><input
                                    type="checkbox" name="cart_jumal" id="jumal"
                                    style="transform: scale(1.5);" value="jumal"
                                    onclick='selectjumal(this)'>&nbsp;&nbsp;주말선택</label>
                              </p>
   <p>
      ...
      <div class="product-details d-md-flex">
         <div class="input-group d-flex">
            ...
            <input type="text" id="cart_cnt_${c.cart_code}" name="cart_cnt" value="${c.cart_cnt}" class="form-control input-number" value="1" min="1" max="100">
    ...
    <script>
      $(document).ready(function() {
    	  $('.quantity-right-plus').click(function(e){
            e.preventDefault();
            var cart_cnt = parseInt($('#cart_cnt_${c.cart_code}').val());
            $('#cart_cnt_${c.cart_code}').val(cart_cnt + 1);
         });
            ...
            if(cart_cnt>0){
               $('#cart_cnt_${c.cart_code}').val(cart_cnt - 1);
                  ...
                  <div class="modal-footer">
                     ...
                     <button type="button" class="btn btn-black py-3 px-5"
...
function daycheck(daycheck) {
   const checkboxes4
    = document.getElementsByName('cart_day');
   const count = checkboxes4.lenght;
