...
public class LoginController {
	...
	public String login(MemberVO memberVO, Model model, HttpServletRequest request, HttpSession session) throws Exception {
		...
		if (lvo != null) { // 일치하는 아이디 존재시
			...
			String encodePw = lvo.getMem_pw(); // db에 저장한 인코딩된 비번
			...
			if (true == pwEncoder.matches(rawPw, encodePw)) {  // 비번 일치 여부 판단
				...
				logger.info("로그인 성공");
				session.setAttribute("message", "로그인 성공하였습니다!");
